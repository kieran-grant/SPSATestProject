...
from wrappers.base_dafx_wrapper import DAFXWrapperBase

class GenericDAFXWrapper(DAFXWrapperBase):
    ...
    def apply_normalised_index_parameter(self, idx, value):
        ...
        self.apply_normalised_named_parameter(param_name, value)
    ...
    def apply_normalised_named_parameter(self, name, value):
        ...
        self.apply_unnormalised_named_parameter(name, raw_val)
    ...
    def apply_normalised_parameter_vector(self, vector):
        ...
        for idx in range(len(vector)):
            self.apply_normalised_index_parameter(idx, vector[idx])

    def apply_unnormalised_index_parameter(self, idx, value):
        """
        Apply an unnormalised value to a parameter by index
        """
        param_name = self._get_name_from_index(idx)
        self.apply_unnormalised_named_parameter(param_name, value)

    def apply_unnormalised_named_parameter(self, name, value):
        """
        Apply an unnormalised value to a parameter by name
        """
    ...
    def apply_unnormalised_parameter_vector(self, vector):
        """
        Apply a vector of unnormalised values to all parameters
        """
        self._assert_valid_vector_length(vector)

        for idx in range(len(vector)):
            self.apply_unnormalised_index_parameter(idx, vector[idx])

    def get_current_param_settings(self):
        """
        Returns parameter names and their unnormalised values
        """
        param_dict = {}
